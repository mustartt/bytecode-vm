...
void bytecode_format::serialize(std::ostream &os) {
    ...
    uint64_t data_comment_pool_size = data_comments.size();
    os.write(reinterpret_cast<const char *>(&data_comment_pool_size), sizeof(uint64_t));
    for (const auto &entry: data_comments) {
        os.write(reinterpret_cast<const char *>(&entry.first), sizeof(std::size_t));
        write_string(os, entry.second);
    }

    // Serialize const comments
    uint64_t const_comment_pool_size = const_comments.size();
    os.write(reinterpret_cast<const char *>(&const_comment_pool_size), sizeof(uint64_t));
    for (const auto &entry: const_comments) {
        os.write(reinterpret_cast<const char *>(&entry.first), sizeof(std::size_t));
        write_string(os, entry.second);
    }
}
...
void bytecode_format::deserialize(std::istream &is) {
    ...
    // Deserialize data comments
    uint64_t data_comment_pool_size;
    is.read(reinterpret_cast<char *>(&data_comment_pool_size), sizeof(uint64_t));
    data_comments.clear();
    for (uint64_t i = 0; i < data_comment_pool_size; ++i) {
        std::size_t key;
        std::string value;
    ...
    // Deserialize const comments
    uint64_t const_comment_pool_size;
    is.read(reinterpret_cast<char *>(&const_comment_pool_size), sizeof(uint64_t));
    const_comments.clear();
    for (uint64_t i = 0; i < const_comment_pool_size; ++i) {
        std::size_t key;
        std::string value;
        is.read(reinterpret_cast<char *>(&key), sizeof(std::size_t));
        read_string(is, value);
        const_comments[key] = value;
    }
}
